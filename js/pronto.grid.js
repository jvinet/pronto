...
function grid_dispatch(evt) {
	...
	$.get(url, {_ajax: 1}, function(data){
		var $tr = $(targ).parent().parent().parent();
		if(data.html) grid_loadhtml($tr, data.html);
 ...
 * AJAX-enabled.
 *
 * The 'targ' argument should be the <TR> element that will contain
 * the new content.
 */
function grid_loadhtml(targ, html) {
	var tr_id = $(targ).attr('id');
	var td_id = "#"+tr_id+"_form_td";
	var close_str = "<br /><br />"+
 ...
 *
 * The 'targ' argument should be the <TR> element that will be removed.
 */
