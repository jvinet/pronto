 ...
 * Modified for Pronto
 */
if(!defined('_TINYBROWSER_CONFIG')) {
	// To define your own application-specific configuration, create your
	// own tinybrowser config file and include() it before this one.  Make
	// sure to define _TINYBROWSER_CONFIG.
	...
	*/
	...
	//if(isset($_GET['sessidpass'])) session_id($_GET['sessidpass']); // workaround for Flash session bug
	//session_start();
	$tinybrowser['sessioncheck'] = 'TINYMCE'; //name of session variable to check

	// Random string used to secure Flash upload if session control not enabled - be sure to change!
	$tinybrowser['obfuscate'] = 'haesh2dae1ZiengeaF0phu5vohwai9oz';
	...
	$tinybrowser['path']['image'] = DIR_WS_BASE.'/img/content/'; // Image files location - also creates a '_thumbs' subdirectory within this path to hold the image thumbnails
	$tinybrowser['path']['media'] = DIR_WS_BASE.'/img/content/'; // Media files location
	$tinybrowser['path']['file']  = DIR_WS_BASE.'/img/content/'; // Other files location
